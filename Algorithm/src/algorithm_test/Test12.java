...
public class Test12 {
	...
	public static int[] solution(int[] array, int[][] commands) {
        ...
        for (int[] nums : commands) {
        	int[] value = new int[nums[1]-nums[0]+1];
        	int index = 0;
        	...
        	Arrays.sort(value);
        	answer[a++] = value[nums[2]-1];
        }
	
	// 다른 풀이
//	public static int[] solution(int[] array, int[][] commands) {
//        int[] answer = new int[commands.length];
//        for (int i=0; i<commands.length; i++) {			// 시작 인덱스		// 배열 크기
//       	int[] value = Arrays.copyOfRange(array, commands[i][0]-1, commands[i][1]);
//        	Arrays.sort(value);
//       	answer[i] = value[commands[i][2]-1];
//        }
